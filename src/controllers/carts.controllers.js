...
const purchaseCart = async (req,res) =>{
  try {
    ...
    if(!cart) return res.status(401).json({status: "error", response: `Cart  with ${cid} not found` });//Si el carrito no se encuentra devuelve error 
    ...
    if (!ticket) throw new Error('Ticket creation failed');//Si el ticket no se crea arroja error
